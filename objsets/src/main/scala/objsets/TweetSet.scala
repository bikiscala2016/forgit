...
abstract class TweetSet {
   ...
   */
    def union(that: TweetSet): TweetSet = 
      filterAcc( (t: Tweet) => if(this.contains(t) || that.contains(t)) true else false , that )
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    ...
    def loop( ts: TweetSet, acc: Tweet ): Tweet = {
    if( ts.isEmpty ) acc
    else if( ts.mostRetweeted.retweets > acc.retweets ) ts.mostRetweeted else acc
    }
    
    loop(left,loop( right, elem))
  }
